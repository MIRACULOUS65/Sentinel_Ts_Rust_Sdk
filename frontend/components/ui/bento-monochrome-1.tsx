...
function Bento3Section({ items = flows, metrics = [], title, subtitle, badge }: Bento3SectionProps) {
    ...
    // Use explicit typing for casting
            ...
            >
                ...
                <header className="flex flex-col gap-10 md:flex-row md:items-end md:justify-between">
                    <div className="space-y-5">
                        ...
                        <h1 className={`text-3xl font-semibold leading-tight sm:text-4xl md:text-5xl ${palette.heading}`}>
                            {title || "End-to-End Risk Infrastructure Visualization"}
                        </h1>
                        <p className={`max-w-2xl text-sm sm:text-base md:text-lg ${palette.muted}`}>
                            {subtitle || "Monitor risk scoring, oracle signing, and smart contract enforcement in real-time."}
                        </p>
                    </div>

                    <div className="flex flex-col items-start gap-4 md:items-end">
                        <div className="flex flex-wrap items-center gap-3">
                            <span className={`text-xs uppercase tracking-[0.35em] ${palette.muted}`}>Theme</span>
                            <button
                                type="button"
                                onClick={toggleTheme}
                ...
                <footer className="bento3-footer flex flex-col gap-5 border-t border-dashed border-current/20 pt-8 text-sm md:flex-row md:items-center md:justify-between">
                    <div className={`flex flex-col gap-2 ${palette.muted}`}>
                        <span className="text-xs uppercase tracking-[0.35em]">System Status</span>
                        <span className="text-base font-medium text-current">All systems operational.</span>
                    </div>
                </footer>
            </section>
        </div>
...
function FlowCard({ flow, palette, index, visible }: { flow: BentoItem; palette: any; index: number; visible: boolean }) {
    ...
    if (flow.children) {
        return (
            <article
                ...
                style={{
                    "--bento3-delay": `${index * 90}ms`,
                    gridColumn: flow.colSpan === 2 ? "span 2" : "auto"
                } as React.CSSProperties}
            >
                {flow.children}
            </article>
        ...
        >
            <div className="relative flex flex-col gap-5 lg:flex-row lg:items-start">
                ...
                <div className={`mt-4 flex h-12 w-12 shrink-0 items-center justify-center rounded-full border ${palette.cardBorder} ${palette.card} sm:h-14 sm:w-14 lg:ml-auto lg:mt-0 lg:h-16 lg:w-16`}>
                    <AnimatedIcon variant={flow.variant || "orbit"} />
                </div>
            </div>
